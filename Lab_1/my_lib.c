...
#define ERROR_CODE 1
#define REG_EXIT_CODE 0
#define EXPECTED_CHUNK_NUMBER 2
...
float count_amp(float* pars,int stride,int bps,int frequency){
	int lr_sample_number= (stride/bps)/CHANNELS_NUMBER;
	float channel_stride = ((stride/bps)%2==1?PI:0);
	float dist = cos(channel_stride+2*PI*((float)lr_sample_number*pars[1])/frequency);
	// +1 to get values between 0 and 2
	// /2 to make [0;1] values
	dist=(dist+1)*pars[0]/2;
	return dist;
}
...
char update_data(work_mode mode,sub_chunk* wav_file){
	...
	switch(mode){
		...
		//trembling effect
		case TREMBLE:
			//requires 2 parameters: amplitude and frequency
			par = calloc(2,sizeof(float));
			printf("Enter amplification c-ent[0.1:10]:");
			scanf("%f",&par[0]);
			fflush(stdin);
	...
	for(unsigned long int i= 0 ; i<=wav_file[1].chunkSize; i+=bps){
		...
		switch(mode){
			...
			break;
			case TREMBLE:
				*samp_data = *samp_data*(count_amp(par,i,bps,freq));
			break;
		}
...
char read_wav(const char* filename){
	...
	sub_chunk* wav_file= calloc(EXPECTED_CHUNK_NUMBER,sizeof(sub_chunk));
	...
	for(char i =0;i<2;i++){
		...
		printf("Chunk size:%d|read:",wav_file[i].chunkSize);
